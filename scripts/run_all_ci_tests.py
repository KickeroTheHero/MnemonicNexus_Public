...
def generate_test_report(results: List[dict]) -> str:
    ...
    criteria_checks = [
        (
            "S0 evidence artifacts are green",
            "ci:s0:snapshot-and-hash" in [r["name"] for r in results if r["success"]],
        ),
        (
            "409 idempotency proof verified",
            "ci:s0:dupe-409-golden" in [r["name"] for r in results if r["success"]],
        ),
        (
            "EMO base active via translator",
            "ci:emo:translator-parity" in [r["name"] for r in results if r["success"]],
        ),
        (
            "Identity, versioning, and lineage verified",
            "ci:emo:lineage-integrity" in [r["name"] for r in results if r["success"]],
        ),
    ]
