...
async def test_duplicate_409():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if response1.status_code != 201:
            print(f"❌ First submission failed. Expected 201, got {response1.status_code}")
            return False
        ...
        if response2.status_code != 409:
            print(f"❌ Second submission should return 409, got {response2.status_code}")
            return False
        ...
        async with asyncpg.create_pool(database_url, min_size=1, max_size=2) as pool:
            async with pool.acquire() as conn:
                ...
                if str(event["event_id"]) != event_id_1:
                    print(f"❌ Event ID mismatch. Expected {event_id_1}, got {event['event_id']}")
                    return False
