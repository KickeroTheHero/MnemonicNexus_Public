        ...
        async with pool.acquire() as conn:
            ...
            try:
                ...
                graph_name = await conn.fetchval(
                    "SELECT lens_graph.ensure_graph_exists($1::uuid, $2)", world_id, branch
                )
...
def compute_determinism_state_hash(snapshots: Dict[str, Any]) -> str:
    ...
    canonical_json = json.dumps(sorted_snapshots, default=serialize_datetime, sort_keys=True)
        ...
        result = {
            ...
            "lens_summary": {
                ...
                "semantic": {
                    ...
                    "emo_embeddings": len(snapshots["semantic"].get("emo_embeddings", [])),
                },
                "graph": {
                    "node_count": snapshots["graph"].get("node_count", 0),
                    "edge_count": snapshots["graph"].get("edge_count", 0),
                    "emo_nodes": snapshots["graph"].get("emo_graph_stats", {}).get("total_nodes", 0),
                }
            }
        }
