    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            return HybridSearchResponse(
                ...
                k=request.k,
                rank_version=RANK_VERSION,
                fusion_method="weighted_sum" if request.mode == "hybrid" else "single_source",
...
async def _hybrid_graph_search(
    ...
    if related_emos:
        ...
        for content_row in related_content:
            ...
            depth = next(
                (r["depth"] for r in related_emos if r["emo_id"] == emo_id),
                1
            )
            ...
            seed_results.append(SearchResult(
                emo_id=emo_id,
